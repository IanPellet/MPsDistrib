...
%alpha = [0];
alpha = 1.45;
leg = arrayfun(@(i) num2str(i), alpha, 'UniformOutput', false);

mse_list = ones(size(alpha));
...
for k = 
    ...
    figure(3)
    [Ca, MSEa, z] = transport(a);
    %Cai = interp1(zn, Cn, z_max, 'pchip');
...
function [C, mse, z_] = transport(alpha)
