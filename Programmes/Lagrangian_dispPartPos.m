...
nStepF = tf/dt+1;
if cast(nStepF, 'uint32') ~= nStepF
    nStepF = cast(nStepF+1, 'uint32');
end
T = ones(nStep,1);
ChNorm = ones(nStep,N);
ppHist = ones(nStep,nPart);
ppT = ones(nStep,1);
...
for i=1:nStep
    ...
    ylabel('Depth (m)')
    ylim([-L 0])
    title(['Time = ' num2str(ppT(i,1)) 's'])
    fname = [path num2str(i) '.png'];
 ...
 images    = cell(nStep,1);
 for i=1:nStep
    images{i} = imread([path num2str(i) '.png']);
 end, clear i,

 vidName = ['D' num2str(D) '_rho' num2str(rhoP) '_nPart' num2str(nPart)...
     '_tf' num2str(tf) '_dttest' num2str(dt_test)];
 
 % create the video writer with 1 fps
 writerObj = VideoWriter(['../Results/PosPart/' vidName '.avi']);
 writerObj.FrameRate = 10;
 % set the seconds per image
 secsPerImage = ones(1,nStep)*2;
 % open the video writer
 open(writerObj);
 % write the frames to the video
 for u=1:fix(length(images))
