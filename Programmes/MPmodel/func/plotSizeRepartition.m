function [resE] = plotSizeRepartition(D_test, RhoP, nDepth, saveFig, windSpeed)
...
for i=1:length(D_test)
    D = D_test(i);
    [resE(i).C, resE(i).z] = Transport_Eulerian(D, RhoP, N, L, false, windSpeed, datetime('3/18/2021'));
    resE(i).D = D;
    resE(i).C = resE(i).C';
...
legend(plots)
%%% Rainbow colorMap %%%%%%%%%%%
c = nToColorMap(length(plots));
set(plots,{'FaceColor'},c)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ylim([-5 0])
